...
export function ColumnGlossary({ items }: { items: ColumnNote[] }) {
  return (
    ...
    <details className="group border-b border-hairline">
      ...
      <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-3 text-body font-medium text-muted transition-colors hover:text-holo">
      ...
      */}
      <dl className="border-t border-hairline-soft px-5 py-4 text-body leading-relaxed sm:columns-2 sm:gap-9 sm:[column-rule:1px_solid_var(--color-hairline)]">
        {items.map((item) => (
          <div
            key={item.term}
